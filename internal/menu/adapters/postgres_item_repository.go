...
func (r *PostgresItemRepository) Save(item *menu.MenuItem) error {
	_, err := r.db.Exec(
		...
		item.PhotoURL, item.PhotoOriginalURL, item.IsAvailable,
...
func (r *PostgresItemRepository) FindByCategoryID(categoryID common.CategoryID) ([]*menu.MenuItem, error) {
	return r.queryItems(`SELECT id, category_id, restaurant_id, name, description, price, photo_url, photo_original_url, is_available, created_at, updated_at FROM menu_items WHERE category_id = $1`, string(categoryID))
}

func (r *PostgresItemRepository) FindByRestaurantID(restaurantID common.RestaurantID) ([]*menu.MenuItem, error) {
	return r.queryItems(`SELECT id, category_id, restaurant_id, name, description, price, photo_url, photo_original_url, is_available, created_at, updated_at FROM menu_items WHERE restaurant_id = $1`, string(restaurantID))
}

func (r *PostgresItemRepository) FindAvailableByRestaurantID(restaurantID common.RestaurantID) ([]*menu.MenuItem, error) {
	return r.queryItems(`SELECT id, category_id, restaurant_id, name, description, price, photo_url, photo_original_url, is_available, created_at, updated_at FROM menu_items WHERE restaurant_id = $1 AND is_available = true`, string(restaurantID))
}

func (r *PostgresItemRepository) Update(item *menu.MenuItem) error {
	result, err := r.db.Exec(
		`UPDATE menu_items SET category_id=$2, name=$3, description=$4, price=$5, photo_url=$6, photo_original_url=$7, is_available=$8, updated_at=$9 WHERE id=$1`,
		string(item.ID), string(item.CategoryID), item.Name, item.Description, item.Price,
		item.PhotoURL, item.PhotoOriginalURL, item.IsAvailable, item.UpdatedAt)
	if err != nil {
		return err
	}
