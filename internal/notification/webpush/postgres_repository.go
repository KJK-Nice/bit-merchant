...
func (r *PostgresRepository) Upsert(sub *Subscription) error {
	...
	// ON CONFLICT target matches push_subscriptions_unique_idx (NULLS NOT
	// DISTINCT) — keyed on the full scope (endpoint, role, order/restaurant)
	// so the same browser endpoint can hold concurrent subscriptions for
	// multiple orders without overwriting.
	_, err := r.db.Exec(`
		INSERT INTO push_subscriptions (role, order_number, restaurant_id, endpoint, auth_key, p256dh_key)
		VALUES ($1, $2, $3, $4, $5, $6)
		ON CONFLICT (endpoint, role, order_number, restaurant_id) DO UPDATE SET
